...
- **Соответствие** — AG Grid, зеркалящий сетку «Инвентарь»: боковая панель фильтров, сохранённая видимость колонок и сортировка, полнотекстовый поиск и панель деталей на каждую находку. Строки несут статус, статью, категорию, требование, описание разрыва, рекомендации по устранению и доказательства. Маленький чип **Обнаружено ИИ** подсвечивает карточки, помеченные семантическим детектором как содержащие ИИ, даже если они не отмечены как ИИ-подтипы.
...
- CVE-находки апсертятся по `(card_id, cve_id)`. Заданный пользователем статус (`acknowledged`, `mitigated`, `accepted`) и любые обратные ссылки на продвинутый Риск переживают следующее сканирование.
- Находки соответствия апсертятся по `(scope, card, regulation, normalised_article)`. Идентификаторы статей вроде *Art. 6 / Article 6 / § 6* сворачиваются в одну строку, так что переформулировки LLM больше не создают дубликаты.
- Находка, которую следующий проход больше не сообщает, **не удаляется** — она помечается `auto_resolved=true` и по умолчанию скрывается, поэтому её история (и связанный продвинутый Риск) остаются нетронутыми.
- **ИИ-вердикт** пользователя по карточке (`hasAiFeatures = true / false`) также сохраняется. Если вы подтвердили или отвергли ИИ-классификацию LLM, это решение перекрывает детектор при последующих сканированиях — дрейф LLM не может тихо изменить охват.
...
CVE-находки и находки соответствия имеют **разные жизненные циклы**, каждый из которых отображается как горизонтальная фазовая шкала в панели находки. Оба ограничены пользователями с `security_compliance.manage`; движок принудительно проверяет переходы на сервере и отклоняет недопустимые движения с понятной ошибкой.
