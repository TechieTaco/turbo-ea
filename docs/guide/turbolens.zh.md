...
- **合规** —— 与清单网格一致的 AG Grid：过滤侧栏、可持久化的列可见性与排序、全文搜索，以及每条发现的明细抽屉。行中包含状态、条款、类别、要求、差距描述、修复建议与证据。一个小小的 **AI 检测**标签会标出语义检测器识别为承载 AI 的卡片，即使它们未被标记为 AI 子类型。
...
- CVE 发现以 `(card_id, cve_id)` 为键进行 upsert。用户设置的状态（`acknowledged`、`mitigated`、`accepted`）以及到已晋升风险的反向链接都会在下次扫描中保留。
- 合规发现以 `(scope, card, regulation, normalised_article)` 为键进行 upsert。诸如 *Art. 6 / Article 6 / § 6* 的条款标识会折叠到同一行，因此 LLM 的措辞变化不再产生重复行。
- 下一次扫描不再报告的发现**不会被删除** —— 它会被标记为 `auto_resolved=true` 并默认隐藏，因此其历史（以及关联的已晋升风险）保持完整。
- 用户对卡片的 **AI 判定**（`hasAiFeatures = true / false`）同样保留。无论你确认还是否决 LLM 的 AI 分类，该决定都会在后续扫描中覆盖检测器 —— LLM 的漂移无法悄然改变适用范围。
...
CVE 与合规发现具有**不同的生命周期**，并各自在发现抽屉中以横向阶段时间线呈现。两者均限于具备 `security_compliance.manage` 的用户；引擎在服务端强制过渡，并以清晰错误拒绝非法移动。
