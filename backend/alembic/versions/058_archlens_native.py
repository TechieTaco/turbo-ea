"""ArchLens native integration — new tables, rework analysis_runs, drop connections.
...
def upgrade() -> None:
    ...
    # ── Rework analysis_runs: drop FK to connections, make standalone ──────
    op.drop_constraint(
        "archlens_analysis_runs_connection_id_fkey",
...
def downgrade() -> None:
    ...
    # Re-add FK on analysis_runs
    op.drop_column("archlens_analysis_runs", "updated_at")
    op.drop_column("archlens_analysis_runs", "created_at")
    op.alter_column("archlens_analysis_runs", "connection_id", nullable=False)
