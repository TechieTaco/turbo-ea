...
- **Public API surface is auto-documented**: `docs/api/openapi.json` is generated from `app.main:app` by `scripts/dump_openapi.py` and rendered in the user manual at `/admin/api/` via Scalar. Do not hand-edit it. After any change to a route, request/response schema, or `/VERSION`, run `python scripts/dump_openapi.py` and commit the result — CI fails PRs whose spec has drifted, and pushes to `main` auto-regenerate it via the `openapi-spec` workflow job.
