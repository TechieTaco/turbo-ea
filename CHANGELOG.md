...
- **Bulk archive / bulk delete no longer trip Chrome's `ERR_INSUFFICIENT_RESOURCES`.** Selecting thousands of cards in the Inventory and clicking *Archive* or *Delete* used to fan out one parallel `POST /cards/{id}/archive` (or `DELETE /cards/{id}`) per selected card. Chrome's per-origin socket pool exhausts long before a workload of that size lands, so the requests failed before leaving the browser. The dialog now drains the selection through a 5-worker pool, shows a `done/total` progress bar, and on partial failure keeps the dialog open with a "X of Y archived; Z failed" summary instead of silently swallowing the errors.
- **Bulk archive / bulk delete no longer report cascade-race responses as failures.** The per-card archive and delete endpoints are transactional with cascades — archiving a parent flips its whole subtree to ARCHIVED in one transaction. With the new 5-worker pool, a sibling worker can reach a descendant after that cascade has already happened; the backend then correctly returns `400 "Card is already archived"` or `404 "Card not found"`, but those responses describe an end state that *matches* the user's intent. The dialog now treats them as idempotent no-ops rather than counting them as failures, so a 1050-card selection where 243 land via cascade reports "1050 of 1050" instead of "807 of 1050; 243 failed".
