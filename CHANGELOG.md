...
- Dependencies Report: long-pressing a card to re-centre the Layered Dependency View no longer leaves the new graph dimmed (it looked empty until the user refreshed the page). The hover-dimming class set when the user hovered the long-pressed card was not cleared because long-press fires `onNodeShiftClick` directly from the pointer-down timer, bypassing the click handler that would normally clear it. Long-press now goes through a wrapper that resets the hover state before navigating, matching the shift+click behaviour. Also fixes the same path in the Card Detail dependency section.
