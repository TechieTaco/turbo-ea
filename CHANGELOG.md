...
### Added
- **PPM Gantt — smoothed dependency arrows.** The library renders dependency arrows as a hard-coded 5-segment staircase with sharp 90° corners. We now post-process the SVG path on every render via a small MutationObserver: when the predecessor's right edge clears the successor's left edge (the common forward case) the staircase collapses to a single L-elbow at the gap midpoint, and when the rows overlap (the loop-back case) the four corners are softened with quadratic curves of fixed pixel radius. Click-zone paths are rewritten in the same pass so the hit-target follows the visible curve. Rounded `stroke-linejoin` / `stroke-linecap` are applied via CSS for the residual straight segments.
