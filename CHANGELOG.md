...
- Codespaces `postCreateCommand` is now resilient to transient build failures. The frontend image clones `jgraph/drawio` (~50 MB) and runs `npm ci` during build — both can fail on a flaky Codespaces network and used to abort the script with a non-zero exit (the red cross next to the postCreateCommand step), leaving the codespace with no containers and the forwarded port returning 502. The script now retries the build once on failure, splits build from up so a partial failure still produces diagnostics, always exits 0, and prints `docker compose ps` + recent backend/frontend logs whenever the readiness probe times out, so the user can see what to fix instead of staring at an opaque 502.
