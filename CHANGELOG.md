...
- **Diagram editor / viewer top toolbar hidden on iPad.** The Back / Edit / Sync bar at the top of the diagram pages was hidden behind the AppBar on iPad Safari while the URL bar was visible. Switched the page-level height calc from `100vh` to `100dvh` (with a `@supports` fallback to `vh` for older browsers) and dropped the negative top margin so the toolbar always sits below the fixed AppBar. Affects both `DiagramEditor` and `DiagramViewer`.
