...
- **PPM Gantt — completion chip is now editable on leaf work packages with tasks.** Clicking the percentage chip on a leaf WBS that has tasks now opens the slider popover instead of being locked. The chip stays read-only on true hierarchy parents (work packages that contain other work packages), where the value is always rolled up from the subtree.
