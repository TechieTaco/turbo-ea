...
`docs/api/openapi.json` powers the **API Reference** page (`docs/admin/api.md`), which embeds [Scalar](https://github.com/scalar/scalar) to render the live spec in the user manual. The file is **regenerated by CI** on every push to `main` that touches the backend, but you can refresh it locally any time:
